  ...
  $.ajax({
    ...
    .then(function(response) {
      ...
      breeds.forEach(element => {
        $('#breed').append(
          `<option value="${element.$t}">${element.$t}</option>`
        );
      });
    });
};
...
dogApp.userPreferences = function() {
  ...
  $('#animal-finder').submit(function(event) {
    ...
    // Dog Location
    dogApp.userAnswers.dogLocationAnswer =
      dogApp.userAnswers.dogCityAnswer +
      ',' +
      dogApp.userAnswers.dogProvinceStateAnswer;

    // Breed
    dogApp.userAnswers.breedAnswer = $('#breed')
      .find(':selected')
      .val();

    // Age
    dogApp.userAnswers.ageAnswer = $('#age')
      .find(':selected')
      .val();

    // Size
    dogApp.userAnswers.sizeAnswer = $('#size')
      .find(':selected')
      .val();

    // Gender
    dogApp.userAnswers.genderAnswer = $('#gender')
      .find(':selected')
      .val();
    ...
    $.ajax({
      ...
      if (data.petfinder.pets === undefined) {
        swal(
          "Please make sure you've entered a City and Province/State in North America."
        );
        return;
      }
...
dogApp.filterOptions = function(data) {
  ...
  dogApp.optionsSelectedArray.forEach(element => {
    ...
    dogApp.optionsFilteredArray = dogApp.optionsFilteredArray.filter(
      element => {
        ...
        const housetrained = optionsArray.filter(element => element.$t === filterValue)
...
dogApp.displayDogs = function() {
  dogApp.optionsFilteredArray.forEach(function(dog) {
    ...
    const location = $('<p>').text(
      `${dog.contact.city.$t}, ${dog.contact.state.$t}`
    );
    const button = $('<a>')
      .addClass('button')
      .attr('href', `https://www.petfinder.com/petdetail/${dog.id.$t}`)
      .attr('target', 'blank')
      .text(`Meet your Mutt Match`);
    const contentFlex = $('<div>')
      .addClass('content-flex-container')
      .append(name, location, button);
    const imageDiv = $('<div>')
      .addClass('image-container')
      .append(image);
    const gridItem = $('<div>')
      .addClass('grid-item')
      .append(imageDiv, contentFlex);
